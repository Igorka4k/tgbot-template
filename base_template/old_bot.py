...
from functions.payments.example import payment_connect, pay_carousel_connect, carousel_editor_welcome, \
    invoice_editor_message_handler

from telegram.ext import Updater, CommandHandler, Filters, MessageHandler, ConversationHandler, RegexHandler, \
    StringRegexHandler, CallbackQueryHandler
...
def certificate_script(update, ctx):
    if ctx.user_data["is_admin"]:
        ...
        carousel_editor_welcome(update, ctx)
...
def stop(update, ctx):
    """dialog breaking"""
    keyboard = ReplyKeyboardMarkup([["/menu"]], resize_keyboard=True)
    ctx.bot.send_message(chat_id=update.effective_chat.id, text="Вы принудительно вернулись в начало.",
                         reply_markup=keyboard)
...
def all_messages_handler(update, ctx):
    msg = update.message.text
    if msg in ("Просмотр позиций (в режиме редактора)", "Добавление позиции", "Изменение позиции", "Удаление позиции"):
        return invoice_editor_message_handler(update, ctx)


def test(update, ctx):
    ctx.bot.send_message(chat_id=update.effective_chat.id, text="test")
    # return ConversationHandler.END
...
invoice_editor = MessageHandler(Filters.text & (~Filters.command), invoice_editor_message_handler)

invoice_editor_conv_handler = ConversationHandler(
    entry_points=[invoice_editor],
    states={
        "test": [MessageHandler(Filters.regex('Изменение позиции') & (~Filters.command), test)],
    },
    fallbacks=[stop_handler, main_menu_handler]
)
...
main_menu_conv_handler = ConversationHandler(
    ...
    states={
        ...
        "certificate_script_test": [MessageHandler(Filters.text & (~Filters.command), certificate_script)],
        "notifies_script": [MessageHandler(Filters.text & (~Filters.command), notifies_script)],
        "stop_conversation": [stop_handler],
    },
    fallbacks=[stop_handler, main_menu_handler]
)
...
dispatcher.add_handler(invoice_editor_conv_handler)
dispatcher.add_handler(MessageHandler(Filters.text & (~Filters.command), all_messages_handler))
